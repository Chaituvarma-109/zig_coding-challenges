...
pub fn main(init: std.process.Init) !void {
    ...
    if (port_str) |p| {
        ...
        const address: net.IpAddress = try .parse(host, port);
        var listener: net.Stream = address.connect(io, .{ .mode = .stream, .protocol = .tcp, .timeout = .none }) catch |err| {
            std.log.err("err: {}\n", .{err});
...
fn vanillaScan(io: Io, host: []const u8, wr: *Io.Writer) !void {
    for (1..65536) |port| {
        ...
        var listener: net.Stream = address.connect(io, .{ .mode = .stream, .protocol = .tcp, .timeout = .none }) catch continue;
        defer listener.close(io);
